...
| **S2629** invoke conditionally | Platform 27 — **effectively a drop pool** (scheduler-api 8, oldcore 4, tool-jetty-listener 4). Commons has an open PR on its only clean file. | **~96% drops**, confirming the commons finding: the only clean shape is a redundant `x.toString()`. Everything else passes a genuine method call and needs an `isXxxEnabled()` guard — a judgement call, not a cleanup. |
...
| **S3457** two distinct shapes | Commons 8 → 1 fixed, 2 permanent drops, 5 blocked by an open PR. Platform 41 untriaged. | **Read the message, not just the rule.** "No need to call `toString()`" is clean (drop the call, the formatter/SLF4J does it). "`%n` should be used in place of `\n`" is a behaviour change — always a drop when the produced string is asserted or compared. |
