...
| **S108** empty block | **Platform-only** (commons/rendering 0) and clustered: 83 in 20 files, 61 of them in four oldcore legacy files (`XWiki` 19, `XWikiRightServiceImpl` 16, `XWikiHibernateStore` 11, `XWikiPluginManager` 10). **82 shipped in one PR**; nearly all are deliberately-empty `catch` blocks. **Cost 4 review rounds** — the only PR of the sweep to need any (see the rule file: the comment must be a TODO). Regenerates from legacy maintenance. | **1% drops (82/83).** Comment-only (the rule ignores a block containing a comment), so the drop condition is truthfulness, and the single drop is a *test helper* swallowing the exception of the call under test. See [rules/java-S108.md](rules/java-S108.md). |
